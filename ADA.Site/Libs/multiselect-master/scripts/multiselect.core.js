...
Multiselect.prototype = {
	...
	_createInputField: function () {
		var input = m_helper.textField({
		    ...
		    class: 'multiselect-input form-control',
		    attributes: {
                readonly: ''
		    }
		}),
		label = m_helper.label({
			...
			class: 'multiselect-count badge badge-pill badge-primary',
			attributes : {
	...
	_createItemList: function () {
		...
		var selectAll = this._createItem('span', -1, 'Tous');
		var result = m_helper.div({
			id : this._getItemListIdentifier(),
			class : 'multiselect-list dropdown-menu'
		});
	...
	_createItem: function (wrapper, value, text, selected) {
	    ...
	    var divCheckBox = m_helper.div({
	        class: 'checkbox'
	    }),
        labelCheckBox = m_helper.label({}),
        spanCheckBox = m_helper.span({ class: 'box' }),
        checkBox = m_helper.checkbox({
	...
	_performSelectItem : function(checkbox, self) {
		...
		if (checkbox.checked) {
			...
			m_helper.setActive(checkbox.parentElement.parentElement.parentElement.parentElement);
		}
		else {
			self._itemCounter--;
			m_helper.deselect(this._item.options[item.index]);
			m_helper.setUnactive(checkbox.parentElement.parentElement.parentElement.parentElement);
		}
	},
	...
	_performSelectAll : function(checkbox, self) {
		...
		if (checkbox.checked) {
			...
			m_helper.each(items, function(e) {
			    m_helper.setActive(e.multiselectElement.parentElement.parentElement.parentElement.parentElement);
			...
			self._itemCounter = 0;
			m_helper.each(items, function(e) {
			    e.multiselectElement.parentElement.parentElement.parentElement.parentElement.classList.remove('active');
